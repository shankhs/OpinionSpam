...
def percentCapitals(text):
	#print 'Calculating percent capitals: '
	nCapitals=0;
	for t in text:
		if t>='A' and t<='Z':
			#print t
...
def megamTrain(featFile,modelFile):
	command = './megam/megam.opt -nc multitron %s >> %s' %(featFile,modelFile)
	print 'command: ',command
	args = [command]
	pid = subprocess.Popen(args,stdin=sys.stdin,stdout=sys.stdout,stderr=sys.stderr,shell=True)
	pid.wait()

def megamTest(modelFile,testFilename):
	ret=[]
...
def extractFeatures(files,featFilename):
	print 'Extracting Jindal Features'
	for f in files:
		...
		f14 = review_score
		#print f
		#print f1,' ',f2,' ',f3,' ',f4,' ',f9,' ',f10,' ',f11,' ',f12,' ',f13,' ',f14
		#raw_input()
...
if __name__=='__main__':
	if len(sys.argv)!=7:
		print 'Enter file containing positive words, file contaning negative words,folder containing reviews, file to store features,file containing list of reviews for which I have to extract features and the model file'
	else:
		...
		#print len(files)
		extractFeatures(files,sys.argv[4])
		megamTrain(sys.argv[4],sys.argv[6])
